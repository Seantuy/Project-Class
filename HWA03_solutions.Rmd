---
title: 'Assignment: HWA03 (solutions)'
author: 'MANOS Thanos'
...
**1.** The text file containing the data is called **priming.txt**. It is available at **your path here**. You can download the data and load them from your project directory into R as a new dataframe called priming by running the following (**make changes according to your paths and OS**):
...
```{r, echo=T, eval=T}
names(priming) <- c("id", "sex", "age", "attention", "prime",
                    "prime.duration", "grandparents", "walk")
```
...
**\textcolor{blue}{ANSWER: v1 and v3 are vectors, while v2 is a dataframe.}**
...
**\textcolor{blue}{ANSWER: vA is a vector while vB is a dataframe.}**
...
**\textcolor{blue}{ANSWER: vmean(vB) doesn't work because you can't take the mean of a dataframe..}**
...
```{r, echo=T, eval=T}
mean(priming_c$walk[priming_c$id <= 50 & priming_c$prime == "elderly"]) - 
  mean(priming_c$walk[priming_c$id <= 50 & priming_c$prime == "neutral"])
```

```{r, echo=T, eval=T}
mean(priming_c$walk[priming_c$id >= 450 & priming_c$prime == "elderly"]) - 
  mean(priming_c$walk[priming_c$id >= 450 & priming_c$prime == "neutral"])
```
...
mean(priming_c$walk[priming_c$grandparents == "yes" & priming_c$prime == "elderly"]) -
  mean(priming_c$walk[priming_c$grandparents == "yes" & priming_c$prime == "neutral"])
```

```{r, echo=T, eval=T}
# No relationship only
mean(priming_c$walk[priming_c$grandparents == "none" & priming_c$prime == "elderly"]) -
  mean(priming_c$walk[priming_c$grandparents == "none" & priming_c$prime == "neutral"])
```
