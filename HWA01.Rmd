...
D. Upload your *PDF* assignment (and Rmd script only when asked) on Teams. If any problems send it to parentheses, like this email [thanos.manos@cyu.com](thanos.manos@cyu.com).
...
max(pirates$height)
```
Answer: The height of the tallest pirate is `r max(pirates$height)` and this is just a demonstration: `r 1+3`.
...
\textcolor{blue}{Answer: The mean and median weight of the pirates is `r mean(pirates$weight)` and  `r median(pirates$weight)` respectively.}
